...
function render() {
  ...
  if (shoppingCount) shoppingCount.textContent = `${state.shoppingList.length} item${state.shoppingList.length === 1 ? "" : "s"}`;
    ...
    if (shoppingLarge) shoppingLarge.innerHTML = shoppingMarkup;
     ...
     if (clearCompleted) clearCompleted.hidden = !state.shoppingList.some((item) => item.checked);
...
function setupUpload() {
  const form = document.querySelector("#upload-form"); const input = document.querySelector("#fridge-photo"); const zone = document.querySelector(".drop-zone"); const preview = document.querySelector("#preview-wrap"); const image = document.querySelector("#preview-image"); const label = document.querySelector("#file-label"); const status = document.querySelector("#scan-status"); const maxPhotoBytes = 12 * 1024 * 1024; const acceptedTypes = new Set(["image/jpeg", "image/png", "image/webp"]); let previewUrl = "";
   function clearPreview() { if (previewUrl) URL.revokeObjectURL(previewUrl); previewUrl = ""; input.value = ""; image.removeAttribute("src"); image.hidden = true; preview.hidden = true; label.textContent = "Drop a photo here"; }
   function showFile(file) { if (!file) return false; if (!acceptedTypes.has(file.type)) { clearPreview(); status.textContent = "Please choose a JPG, PNG, or WebP image."; status.className = "form-status error"; return false; } if (file.size > maxPhotoBytes) { clearPreview(); status.textContent = "That image is larger than 12 MB. Choose a smaller photo."; status.className = "form-status error"; return false; } if (previewUrl) URL.revokeObjectURL(previewUrl); previewUrl = URL.createObjectURL(file); label.textContent = file.name; image.src = previewUrl; image.hidden = false; preview.hidden = false; status.textContent = "Ready to scan."; status.className = "form-status"; return true; }
  ...
  zone.addEventListener("drop", (e) => { const file = e.dataTransfer.files[0]; if (file && showFile(file)) { input.files = e.dataTransfer.files; } });
   document.querySelector("#clear-photo").addEventListener("click", () => { clearPreview(); status.textContent = "Upload a photo to see your latest scan results here."; status.className = "form-status"; });
  image.addEventListener("error", () => { clearPreview(); status.textContent = "That preview could not be loaded. Please choose another image."; status.className = "form-status error"; });
  form.addEventListener("submit", async (event) => { event.preventDefault(); const file = input.files[0]; if (!file || !showFile(file)) return; const button = document.querySelector("#scan-button"); button.disabled = true; button.setAttribute("aria-busy", "true"); button.firstElementChild.textContent = "Reading your fridge..."; status.textContent = "Looking for ingredients and building a week of meals."; status.className = "form-status"; const body = new FormData(); body.append("photo", file); try { const result = await request("/fridge/analyze", { method: "POST", body }); Object.assign(state, result.plan); render(); status.textContent = result.aiUsed ? "Scan complete. Your AI recipes are ready." : `Scan unavailable. ${result.fallbackReason || "Check the model settings and try again."}`; status.className = result.aiUsed ? "form-status success" : "form-status error"; const planNote = document.querySelector("#plan-note"); if (planNote) planNote.textContent = result.aiUsed ? "Personalized by AI vision" : "Scan unavailable"; } catch (error) { status.textContent = error.message; status.className = "form-status error"; } finally { button.disabled = false; button.removeAttribute("aria-busy"); button.firstElementChild.textContent = "Scan my fridge"; } });
}
...
function setupTheme() { const button = document.querySelector("#theme-toggle"); const applyTheme = () => { const dark = state.theme === "dark"; document.body.classList.toggle("dark-mode", dark); button.setAttribute("aria-pressed", String(dark)); button.textContent = dark ? "Light mode" : "Dark mode"; }; button.addEventListener("click", () => { state.theme = state.theme === "dark" ? "light" : "dark"; applyTheme(); saveState(); }); applyTheme(); return applyTheme; }
  async function bootstrap() { if (!window.GizmoAppRuntime) throw new Error("The shared app runtime did not load."); window.GizmoAppRuntime.readConfig(); setupUpload(); setupAddIngredient(); setupNavigation(); setupRecipeActions(); setupShopping(); setupSettings(); const applyTheme = setupTheme(); await loadState(); render(); applyTheme(); window.GizmoAppRuntime.markReady(); }
