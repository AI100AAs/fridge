...
function setupRecipeActions() {
 ...
 function setupTheme() { const button = document.querySelector("#theme-toggle"); const applyTheme = () => { const dark = state.theme === "dark"; document.body.classList.toggle("dark-mode", dark); button.setAttribute("aria-pressed", String(dark)); button.textContent = dark ? "Light mode" : "Dark mode"; }; button.addEventListener("click", () => { state.theme = state.theme === "dark" ? "light" : "dark"; applyTheme(); }); applyTheme(); }
 function bootstrap() { if (!window.GizmoAppRuntime) throw new Error("The shared app runtime did not load."); window.GizmoAppRuntime.readConfig(); setupUpload(); setupAddIngredient(); setupNavigation(); setupRecipeActions(); setupSettings(); setupTheme(); loadState(); window.GizmoAppRuntime.markReady(); }
