const state = { ingredients: [], inventory: [], recipes: [], shoppingList: [], mealPlan: {}, preferences: { dietaryRestrictions: "", allergies: "", preferredCuisines: "", dislikedIngredients: "", servingSize: "", notes: "" } };
...
function setupUpload() {
 ...
 function setupAddIngredient() { const form = document.querySelector("#ingredient-form"); document.querySelector("#inventory-add").addEventListener("click", () => { form.hidden = !form.hidden; if (!form.hidden) document.querySelector("#ingredient-input").focus(); }); form.addEventListener("submit", (event) => { event.preventDefault(); const input = document.querySelector("#ingredient-input"); const quantity = document.querySelector("#quantity-input"); const value = input.value.trim(); if (value) { state.inventory.push({ name: value, quantity: quantity.value.trim(), category: "Fridge" }); state.ingredients = state.inventory.map((item) => item.name); input.value = ""; quantity.value = ""; saveState(); render(); } }); }
...
function bootstrap() { if (!window.GizmoAppRuntime) throw new Error("The shared app runtime did not load."); window.GizmoAppRuntime.readConfig(); setupUpload(); setupAddIngredient(); setupNavigation(); setupRecipeActions(); setupSettings(); loadState(); window.GizmoAppRuntime.markReady(); }
