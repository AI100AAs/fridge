const state = { ingredients: [], recipes: [], shoppingList: [] };
...
function render() {
  ...
  document.querySelector("#week-count").textContent = state.ingredients.length;
  list.innerHTML = state.ingredients.length ? state.ingredients.map((item, index) => `<span class="ingredient-chip">${escapeHtml(item)} <button type="button" data-remove="${index}" aria-label="Remove ${escapeHtml(item)}">×</button></span>`).join("") : `<div class="empty-copy"><span class="empty-emoji">✦</span><p>Scan your fridge to start your ingredient list.</p></div>`;
  list.querySelectorAll("[data-remove]").forEach((button) => button.addEventListener("click", () => { state.ingredients.splice(Number(button.dataset.remove), 1); saveState(); render(); }));
  const recipes = document.querySelector("#recipe-list");
  recipes.innerHTML = state.recipes.length ? state.recipes.map((recipe) => `<article class="recipe-card"><span class="recipe-meta">${escapeHtml(recipe.day || "Flexible")}</span><h4>${escapeHtml(recipe.title)}</h4><p>${escapeHtml(recipe.description)}</p><span class="recipe-tag">${escapeHtml(recipe.time || "30 min")}</span></article>`).join("") : `<div class="empty-wide">Your personalized recipes will appear here after a scan.</div>`;
...
function setupUpload() {
  ...
  form.addEventListener("submit", async (event) => { event.preventDefault(); const file = input.files[0]; if (!file) return; const button = document.querySelector("#scan-button"); button.disabled = true; button.firstElementChild.textContent = "Reading your fridge..."; status.textContent = "Looking for ingredients and building a week of meals."; status.className = "form-status"; const body = new FormData(); body.append("photo", file); try { const result = await request("/fridge/analyze", { method: "POST", body }); Object.assign(state, result.plan); render(); status.textContent = result.aiUsed ? "Scan complete. Your plan is ready." : "Plan ready with a starter read. You can edit anything below."; status.className = "form-status success"; document.querySelector("#plan-note").textContent = result.aiUsed ? "Personalized by AI vision" : "Starter plan · edit to make it yours"; } catch (error) { status.textContent = error.message; status.className = "form-status error"; } finally { button.disabled = false; button.firstElementChild.textContent = "Scan my fridge"; } });
}
function setupAddIngredient() { const form = document.querySelector("#ingredient-form"); document.querySelector("#add-ingredient").addEventListener("click", () => { form.hidden = !form.hidden; if (!form.hidden) document.querySelector("#ingredient-input").focus(); }); form.addEventListener("submit", (event) => { event.preventDefault(); const input = document.querySelector("#ingredient-input"); const value = input.value.trim(); if (value) { state.ingredients.push(value); input.value = ""; saveState(); render(); } }); }
