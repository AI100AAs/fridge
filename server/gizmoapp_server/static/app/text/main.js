...
async function saveState() { if (!userId) return; try { await request("/fridge/state", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(state) }); } catch (_) { /* local UI remains usable if persistence is briefly unavailable */ } }
async function loadState() { if (!userId) return; try { Object.assign(state, await request("/fridge/state")); render(); } catch (_) { render(); } }
